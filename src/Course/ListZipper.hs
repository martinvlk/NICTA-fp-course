...
instance Functor ListZipper where
  f <$> (ListZipper ls z rs) = ListZipper (f <$> ls) (f z) (f <$> rs)
...
toList (ListZipper ls z rs) = reverse ls ++ (z:.Nil) ++ rs
...
withFocus f (ListZipper ls z rs) = ListZipper ls (f z) rs
...
findLeft p (ListZipper ls z rs) =  bool (IsZ $ ListZipper ls' z' rs')
                                   IsNotZ
                                   (isEmpty b)
  where (a, b) = break p ls
        (z':.ls') = b
        rs' = reverse a ++ (z:.Nil) ++ rs
...
findRight p (ListZipper ls z rs) = bool
                                   (IsZ $ ListZipper ls' z' rs')
                                   IsNotZ
                                   (isEmpty b)
  where (a, b) = break p rs
        (z':.rs') = b
        ls' = reverse a ++ (z:.Nil) ++ ls
...
moveLeftLoop (ListZipper Nil z rs) = ListZipper ls' z' Nil
  where (z':.ls') = reverse $ z:.rs
moveLeftLoop (ListZipper ls z rs) = ListZipper ls' z' (z:.rs)
  where (z':.ls') = ls
...
moveRightLoop (ListZipper ls z Nil) = ListZipper Nil z' rs'
  where (z':.rs') = reverse ls ++ (z:.Nil)
moveRightLoop (ListZipper ls z rs) = ListZipper (z:.ls) z' rs'
  where (z':.rs') = rs
...
moveLeft (ListZipper (z':.ls') z rs) = IsZ $ ListZipper ls' z' (z:.rs)
...
moveRight (ListZipper ls z (z':.rs')) = IsZ $ ListZipper (z:.ls) z' rs'
...
swapLeft (ListZipper (l:.ls) z rs) = IsZ $ ListZipper (z:.ls) l rs
...
swapRight (ListZipper ls z (r:.rs)) = IsZ $ ListZipper ls r (z:.rs)
...
dropLefts (ListZipper _ z rs) = ListZipper Nil z rs 
...
dropRights (ListZipper ls z _) = ListZipper ls z Nil 
...
moveLeftN' n lz@(ListZipper ls z rs) | n < 0     = moveRightN' (abs n) lz 
                                     | lls < n   = Left lls
                                     | otherwise = Right $ ListZipper (drop n ls) z' rs'
  where lls = length ls
        (z':.rs') = reverse (take n ls) ++ (z:.Nil) ++ rs
...
moveRightN' 0 lz = Right lz
moveRightN' n lz@(ListZipper ls z rs) | n < 0     = moveLeftN' (abs n) lz 
                                      | lrs < n   = Left lrs
                                      | otherwise = Right $ ListZipper ls' z' rs'
  where lrs = length rs
        (z':.rs') = drop (n-1) rs
        ls' = reverse (take (n-1) rs) ++ z:.ls
...
end lz = case nth (length (lefts lz) + length (rights lz)) lz of
  IsZ lz' -> lz'
...
deletePullLeft (ListZipper (z':.ls') _ rs) = IsZ $ ListZipper ls' z' rs
...
deletePullRight (ListZipper ls _ (z':.rs')) = IsZ $ ListZipper ls z' rs'
...
insertPushLeft z' (ListZipper ls z rs) = ListZipper (z:.ls) z' rs
...
insertPushRight z' (ListZipper ls z rs) = ListZipper ls z' (z:.rs)
...
instance Apply ListZipper where
  (ListZipper lsf zf rsf) <*> (ListZipper lsv zv rsv) = let zap = zipWith id in
    ListZipper (zap lsf lsv) (zf zv) (zap rsf rsv)
...
instance Apply MaybeListZipper where
  IsNotZ <*> IsNotZ = IsNotZ
  _ <*> IsNotZ = IsNotZ
  IsNotZ <*> _ = IsNotZ
  IsZ fs <*> IsZ vs = IsZ $ fs <*> vs
...
instance Applicative ListZipper where
  pure v = let vs = produce id v
           in ListZipper vs v vs
